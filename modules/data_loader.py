...
class DataLoader:
            ...
            if file_type == "csv":
                ...
                try:
                    ...
                    if skip_bad_lines or kwargs.get("auto_detect_issues", True):
                        ...
                        print(
                            f"Successfully loaded CSV with error handling ({len(df)} rows)"
                        )
                    else:
                        ...
                        print(
                            f"Successfully loaded CSV with auto-detected dialect ({len(df)} rows)"
                        )
    ...
    def get_data_info(self, file_path, file_type=None, **kwargs):
        ...
        **kwargs : dict
        ...
        kwargs["skip_bad_lines"] = kwargs.get(
            "skip_bad_lines", True
        )  # Default to skipping bad lines for info
        ...
        for col in df.columns:
            ...
            if pd.api.types.is_numeric_dtype(df[col]):
                col_info.update(
                    {
                        "min": convert_to_native_types(
                            df[col].min() if not pd.isna(df[col].min()) else None
                        ),
                        "max": convert_to_native_types(
                            df[col].max() if not pd.isna(df[col].max()) else None
                        ),
                        "mean": convert_to_native_types(
                            df[col].mean() if not pd.isna(df[col].mean()) else None
                        ),
                        "median": convert_to_native_types(
                            df[col].median() if not pd.isna(df[col].median()) else None
                        ),
                        "std": convert_to_native_types(
                            df[col].std() if not pd.isna(df[col].std()) else None
                        ),
                    }
                )
            ...
            # Add categorical column stats
                ...
                value_counts = {
                    str(k): convert_to_native_types(v) for k, v in value_counts.items()
                }
    ...
    def get_preview_data(self, file_path, file_type, max_rows=10, **kwargs):
        ...
        **kwargs : dict
        ...
        try:
            ...
            kwargs["skip_bad_lines"] = kwargs.get(
                "skip_bad_lines", True
            )  # Default to skipping bad lines for preview
            # Use the enhanced load_data method with all parameters
            df = self.load_data(file_path, file_type, **kwargs)

            # original dataframe shape
            df_original = self.load_data(
                file_path, file_type, skip_bad_lines=False, max_rows=None
            )
    ...
    def save_data(self, df, file_path, file_type=None):
        ...
        if file_type not in self.supported_formats:
            raise ValueError(
                f"Unsupported file format: {file_type}. Supported formats: {self.supported_formats}"
            )
